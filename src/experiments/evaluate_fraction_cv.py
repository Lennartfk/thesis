...
def run_fraction_cv_sweep(config, fractions, methods):
    """
    Evaluates adaptation fractions using a strict 10-fold Sliding Window Cross Validation.
    ...
    for adaptation_name in methods:
        run_name = build_run_name(config.run_name, adaptation_name, "cv_sweep")
        ...
        with mlflow.start_run(run_name=run_name):
            ...
            for fraction in fractions:
                ...
                for fold_index, (subject_id, train_idx, test_idx) in tqdm(
                    ...
                    # Split into 100 blocks randomly for 1% resolution
                    n_total = len(X_test)
                    rng = random.Random(f"{config.seed}:{test_subject_id}")
                    indices = list(range(n_total))
                    rng.shuffle(indices)
                    blocks = np.array_split(indices, 100)
                    
                    run_metrics = []
                    
                    # 10 Runs for Sliding Window (Stride by 10 blocks)
                    for run_i in range(10):
                        start_block = run_i * 10
                        adapt_blocks_idx = [(start_block + j) % 100 for j in range(num_adapt_blocks)]
                        eval_blocks_idx = [b for b in range(100) if b not in adapt_blocks_idx]
                        
                        adapt_indices = np.concatenate([blocks[i] for i in adapt_blocks_idx]) if adapt_blocks_idx else np.array([], dtype=int)
                        eval_indices = np.concatenate([blocks[i] for i in eval_blocks_idx]) if eval_blocks_idx else np.array([], dtype=int)
                    ...
                    avg_metrics.update({
                        ...
                        "adaptation_target_fraction": fraction,
            ...
            if fold_metrics_rows:
                ...
                summary_rows = []
                for fraction in fractions:
                    frac_df = fold_df[np.isclose(fold_df["adaptation_target_fraction"], fraction)]
                    if not frac_df.empty:
                        sum_row = frac_df.mean(numeric_only=True).to_dict()
                        sum_row["adaptation_name"] = adaptation_name
                        sum_row["adaptation_target_fraction"] = fraction
                ...
                for _, row in summary_df.iterrows():
                    step = int(round(row["adaptation_target_fraction"] * 100))
